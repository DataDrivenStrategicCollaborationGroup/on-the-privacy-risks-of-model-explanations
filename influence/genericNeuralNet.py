...
class GenericNeuralNet(object):
    ...
    def get_inverse_hvp_cg(self, v, verbose):
        ...
        fmin_results = minimize(
            ...
            method='Newton-CG',
            jac=fmin_grad_fn,
            hessp=self.get_fmin_hvp,
            callback=cg_callback,
            tol=1e-8,
            options={"maxiter":100}) 

        #print(self.vec_to_list(fmin_results))
        #print(fmin_results2)
        #print(fmin_results2.x- fmin_results)
        return self.vec_to_list(fmin_results.x)
    ...
    def get_influence_on_test_loss(self, test_indices, train_idx, 
        ...
        if force_refresh == False and os.path.exists(approx_filename):
            ...
            if(save): np.savez(approx_filename, inverse_hvp=inverse_hvp)
    ...
    def get_influence_on_example_set_loss(self, example_set, train_idx, 
        approx_type='cg', approx_params=None, force_refresh=True, test_description=["Example"],
        loss_type='normal_loss',
